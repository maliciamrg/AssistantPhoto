...
export const handlers = [
    ...
    rest.get('http://localhost:8099/photos', (req, res, ctx) => {
        return res(
            ctx.status(200),
            ctx.json(allPhotoFromAllIn)
        );
    }),
    rest.get('http://localhost:8099/api/seance-types', (req, res, ctx) => {
        return res(
            ctx.status(200),
            ctx.json(seanceTypesData)
        );
    }),
    rest.get('http://localhost:8099/api/seance-repertoire', (req, res, ctx) => {
        return res(
            ctx.status(200),
            ctx.json(seanceRepertoires)
        );
    }),
    rest.get('http://localhost:8099/api/photos?type=ALL_IN&repertoire=00-CheckIn-2', (req, res, ctx) => {
        return res(
            ctx.status(200),
            ctx.json(seanceRepertoires)
        );
    }),
    rest.get('http://localhost:8099/photos?type=ALL_IN&repertoire=00-CheckIn-2', (req, res, ctx) => {
        return res(
            ctx.status(200),
            ctx.json(allPhotoFromAllIn)
        );
